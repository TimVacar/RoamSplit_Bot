...
EXPENSE_CATEGORIES = ["food", "cafe", "transport", "hotel", "tickets", "shopping", "entertainment", "groceries", "transfer", "excursions", "other"]
...
class TripDB:
    ...
    def set_user_language(self, telegram_user_id: int, language_code: str):
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("UPDATE users SET language_code = %s WHERE telegram_user_id = %s", (language_code, telegram_user_id))
            conn.commit()

    def get_user_language(self, telegram_user_id: int) -> str:
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("SELECT language_code FROM users WHERE telegram_user_id = %s LIMIT 1", (telegram_user_id,))
            row = cur.fetchone()
            return row[0] if row else "en"

    def set_active_trip(self, telegram_user_id: int, trip_id: Optional[int]):
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("UPDATE users SET active_trip_id = %s WHERE telegram_user_id = %s", (trip_id, telegram_user_id))
            conn.commit()

    def get_active_trip(self, telegram_user_id: int) -> Optional[int]:
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("SELECT active_trip_id FROM users WHERE telegram_user_id = %s LIMIT 1", (telegram_user_id,))
            row = cur.fetchone()
    ...
    def get_trip_by_id(self, trip_id: int):
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT id, title, base_currency, trip_language, status, created_at FROM trips WHERE id = %s LIMIT 1",
                (trip_id,),
            )
            return cur.fetchone()
    ...
    def is_trip_member(self, trip_id: int, user_id: int) -> bool:
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT 1 FROM trip_members WHERE trip_id = %s AND user_id = %s AND is_active = TRUE LIMIT 1",
                (trip_id, user_id),
            )
    ...
    def fetch_trip_member_ids(self, trip_id: int):
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(
                "SELECT user_id FROM trip_members WHERE trip_id = %s AND is_active = TRUE ORDER BY joined_at ASC",
                (trip_id,),
            )
    ...
    def fetch_trip_expenses(self, trip_id: int):
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(
                """
                SELECT e.id, e.payer_user_id, e.amount, e.currency, e.category, e.note, e.created_at,
...
async def trip_language_callback(query: CallbackQuery):
    ...
    draft = TripCreateDraft(title=state["title"], currency=state["currency"], trip_language=trip_lang)
...
async def calculate_button(message: Message):
    ...
    lines.append("")
    lines.append("*Balances*" if lang == "en" else "*Балансы*")
    ...
    if not settlements:
        ...
        lines.append("*Who owes whom*" if lang == "en" else "*Кто кому должен*")
        for debtor_id, creditor_id, amount in settlements:
            lines.append(f"• {members.get(debtor_id, str(debtor_id))} → {members.get(creditor_id, str(creditor_id))}: {format_money(amount, currency)}")
    await message.answer("\n".join(lines), parse_mode="Markdown", reply_markup=trip_menu_keyboard(lang))
...
async def split_callback(query: CallbackQuery):
    ...
    if split_mode == "all":
        trip = db.get_trip_by_id(trip_id)
        currency = trip[2] if trip else DEFAULT_TRIP_CURRENCY
        expense_id = db.add_expense(trip_id, query.from_user.id, state["amount"], currency, state["category"], state["note"])
...
async def text_handler(message: Message):
    ...
    if state and state.get("flow") == "add_expense":
        ...
        if state.get("step") == "participant_ids":
            ...
            currency = trip[2] if trip else DEFAULT_TRIP_CURRENCY
            expense_id = db.add_expense(state["trip_id"], user_id, state["amount"], currency, state["category"], state["note"])
            db.add_expense_participants(expense_id, participant_ids)
            PENDING_INPUTS.pop(user_id, None)
...
```bash
git add .
git commit -m "full mvp trip bot with expenses and settlements"
git push
```

Потом дождись деплоя в Railway.
...
И да — **выбор валюты trip** в этом коде есть на шаге создания поездки.  
После названия бот спросит:
```text
Send trip currency. Example: EUR
```

Когда задеплоишь, протестируй такой сценарий:
1. `/start`
2. `Create trip`
3. название
4. `EUR`
...
11. категория
12. комментарий
13. split all / selected
14. `Calculate debts`

Если где-то упрётся, пришли уже **новый лог или точный шаг, где сломалось**.
